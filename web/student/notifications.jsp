<%-- 
    ...
    Created on : Jul 2, 2016, 7:02:35 PM
    Author     : Shardul Rane
--%>
...
<html lang="en">
	...
	<head>
		...
		<title>Tyro | Recent Notification</title>
...
<%
       ...
       PreparedStatement getnoti=con.prepareStatement("select * from notification where s_uid=? and status=? and type=?");
       getnoti.setString(1,uid);
       getnoti.setString(2,"Unseen");
       getnoti.setString(3,"Notice");
       ResultSet notiex=getnoti.executeQuery();
%>
Recent Notices
							...
							<thead>
								<tr>
									...
									<th>Download Action</th>
                                         ...
                                         <%       while(notiex.next())
       {
        int nid=notiex.getInt(7);
        PreparedStatement getnotice=con.prepareStatement("select Title from notices_college where Notice_Id=?");
       getnotice.setInt(1, nid);
       ResultSet shownotice=getnotice.executeQuery();
     %>

                                                            <%  while(shownotice.next())
       {
           
%>
<tr>
                                                <td><%=shownotice.getString("Title")%></td>
						<td><a href="../downloadnotice?id=<%=nid%>"><button type="button" class="btn btn-black">Download <i class="entypo-down"></i> </button></a></td>
</tr>												
<% }%>
...
<%
        ...
        PreparedStatement getnoti1=con.prepareStatement("select * from notification where s_uid=? and status=? and type=?");
       getnoti1.setString(1,uid);
       getnoti1.setString(2,"Unseen");
       getnoti1.setString(3,"Notes");
       ResultSet notiex1=getnoti1.executeQuery();
       %>
     Recent Notes
                                	<table class="table table-bordered responsive">
							<thead>
...
<%
       while(notiex1.next())
       {
        ...
        int nid=notiex1.getInt(7);
        PreparedStatement getnotes=con.prepareStatement("select sub,title from College_notes where n_id=?");
       getnotes.setInt(1, nid);
       ResultSet shownotes=getnotes.executeQuery();
    while(shownotes.next())
       {
           
%>
<tr>                                        
<td><%=shownotes.getString("title")%></td>
                                                <td><%=shownotes.getString("sub")%></td>
						<td><a href="../downloadcode?id=<%=nid%>"><button type="button" class="btn btn-black">Download <i class="entypo-down"></i> </button></a></td>
</tr>												
<% }%>
...
<%
       ...
       PreparedStatement getnoti2=con.prepareStatement("select * from notification where s_uid=? and status=? and type=?");
       getnoti2.setString(1,uid);
       getnoti2.setString(2,"Unseen");
       getnoti2.setString(3,"Papers");
       
       ResultSet notiex2=getnoti2.executeQuery();
       %>
                                                                                Recent Uploaded Question Papers
       ...
       while(notiex2.next())
       {
        ...
        int nid=notiex2.getInt(7);
        PreparedStatement getpapers=con.prepareStatement("select sub,type from question_papers where q_id=?");
       getpapers.setInt(1, nid);
       ResultSet showpapers=getpapers.executeQuery();
       while(showpapers.next())
       {
           
%>
<tr>
                                                        <td><%=showpapers.getString("sub")%></td>
                                                <td><%=showpapers.getString("type")%></td>
						
						
                                                <td><a href="../downloadpaper?id=<%=nid%>"><button type="button" class="btn btn-black">Download <i class="entypo-down"></i> </button></a></td>
</tr>												
<% }%>
				...
				<footer class="main">
                                                                ...
                                                                <%
              PreparedStatement notiup=con.prepareStatement("update notification set status='Seen' where s_uid=?");
              notiup.setString(1,uid);
              notiup.executeUpdate();
        }
