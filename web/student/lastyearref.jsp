<%-- 
    Document   : lastyearref
    Created on : Jun 3, 2016, 11:41:16 AM
    Author     : Shardul Rane
--%>
...
<html lang="en">
	...
	<head>
		...
		<title>Tyro | Referance Papers</title>
...
//var userType= usertype .options[usertype .selectedIndex].value;
//var uname=document.search;
var urls="showsubpapers.jsp?sem="+sem+"&sub="+sub+"&year="+year+"&type="+type;
if (window.XMLHttpRequest)
  {
  xmlhttp=new XMLHttpRequest();
  }
...
xmlhttp.onreadystatechange=function()
  ...
  if (xmlhttp.readyState==4)
    {
        document.getElementById("notesres").innerHTML=xmlhttp.responseText;
     }
  }
xmlhttp.open("GET",urls,true);
xmlhttp.send();
...
function semsend(str)
                    {
                        var urls="getsubdiv.jsp?sem="+str;    
        ...
        var urls="getsubyear.jsp?sub="+str+"&sem="+sem;    
                        var xmlhttp=new XMLHttpRequest();
                        xmlhttp.onreadystatechange=function()
                        {
                                if (xmlhttp.readyState==4 && xmlhttp.status==200)
...
function sendyear(str)
                    {
                        var sem=document.getElementById("sem").value;
                        var sub=document.getElementById("sub").value;
                        var urls="getpapertype.jsp?sub="+sub+"&sem="+sem+"&year="+str;    
                        var xmlhttp=new XMLHttpRequest();
                        xmlhttp.onreadystatechange=function()
                        {
                                if (xmlhttp.readyState==4 && xmlhttp.status==200)
 ...
 <div id="searchres">
                                ...
                                <h2>Get Reference Papers</h2>
				<br/>
	<%
                 
                   String code=getInitParameter("c_code");
           PreparedStatement ps3=con.prepareStatement("select Course,Year from reg_students where uid=?");
           ...
           if(resultset5.next())
           {
                            PreparedStatement ps5=con.prepareStatement("select Distinct Semester from syllabus where Dept_ID=? and Year=?");
                      ps5.setString(1, resultset5.getString("Dept_ID"));
                      ps5.setString(2, Year);
                      ResultSet resultset6=ps5.executeQuery();
                          
          
                   %>	                                
				<div class="panel-body">
								<form role="form" class="form-horizontal form-groups-bordered" name="viewnotes">
								<div class="form-group">
										<label class="col-sm-3 control-label">Semester</label> 
										<div class="col-sm-5">
                                                                                    <select name="sem" id="sem" class="form-control" data-first-option="false" onchange="semsend(this.value)">
											<option value="0">Select Semester</option>
												
                                                                                            <%  while(resultset6.next()){ %>
                                                                                                        <option value="<%= resultset6.getString("Semester")%>"><%= resultset6.getString("Semester")%></option>
                                                                                                        <% } %>	
                                                                                        </select>
										</div>
								</div>
								...
								<div class="form-group">
										...
										<div class="col-sm-5">
                                                                                    <select name="sub" id="sub" class="form-control" onchange="sendsub(this.value)">
											</select>
										</div>
								</div>
                                                                <div class="form-group">
										...
										<div class="col-sm-5">
                                                                                    <select name="years" id="years" class="form-control" onchange="sendyear(this.value)">
											</select>
										</div>
										...
										<div class="col-sm-5">
                                                                                    <select name="type" id="type" class="form-control" onchange="showsubdata()">
											</select>
										</div>
						...
						<div class="row" id="notesres" name="notesres">
