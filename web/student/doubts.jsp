...
xmlhttp.send();
 ...
 <div id="searchres">
                                ...
                                <form role="form" name="notify" action="../submitdoubt" method="post" class="form-horizontal form-groups-bordered">
									<strong>Ask A Doubt</strong> <br /> 
				<%
                                    String uid=(String)request.getSession().getAttribute("uid");
                                    String combo[]=uid.split("-");
                                    String sid=combo[1];
                                PreparedStatement ps3=con.prepareStatement("select * from student_info where sid=? and College_Code=?");
                                ps3.setString(1, sid);
                                ps3.setString(2,combo[0]);
                                ResultSet resultset4=ps3.executeQuery();
                                if(resultset4.next())
                                {
                                    String dept_id=resultset4.getString(3);
                                    String stream=resultset4.getString(5);
                                    String year=resultset4.getString(6);
                                   PreparedStatement ps4=con.prepareStatement("SELECT Paper,Subject FROM syllabus where Dept_ID=? and Year=? And Stream=?");
                                ps4.setString(1, dept_id);
                                ps4.setString(2,year);
                                ps4.setString(3,stream);
                                ...
                                %>
										...
										<div class="col-sm-5"> 
                                                                                    <select name="sub" id="sub" class="select2" data-allow-clear="true" required="required" >
												<option value="0">Select Subject</option>
												<optgroup label="Select Subject">
													
                                                                                                    <%  while(resultset5.next()){ %>
                                                                                                        <option value="<%= resultset5.getString("Paper")+"-"+resultset5.getString("Subject")%>"><%= resultset5.getString("Subject")%></option>
                                                                                                        <% } %>									
												</optgroup>
											</select> 
										</div>
							...
							<div class="form-group">
										<label for="field-ta" class="col-sm-3 control-label">Your Doubt</label> 
                                                                                <div class="col-sm-5"> <textarea class="form-control autogrow" name="doubt" id="doubt" placeholder="Enter Your Doubts" style="overflow: hidden; word-wrap: break-word; resize: horizontal; height: 48px;"></textarea> </div>
						</div>
						<div class="form-group">
								<center>
								<button type="submit" class="btn btn-green btn-icon">
									Submit Your Doubt<i class="entypo-check"></i></button>
									</center>
						</div>
						</form>
                                ...
                                <br>
                                <div class="row">
                                <strong>Your Asked Doubts</strong> <br /> 
                                <div class="col-md-12">
...
<%
    ...
    String cid = (String)request.getSession().getAttribute("cid");
        ...
        PreparedStatement ps5 = con.prepareStatement("SELECT * FROM register_users WHERE uid=? and cid=?");
                    ps5.setString(1, uid);
                    ps5.setString(2, cid);
                    ResultSet rs5 = ps5.executeQuery();
                    if (rs5.next()) {
                        String dept = rs5.getString(6);
                      
            PreparedStatement ps6 = con.prepareStatement("select * from doubts where uid=? and dept_id=? order by asked_date desc");
            ps6.setString(1, uid);
            ps6.setString(2, dept_id);
            ...
            while (rs6.next()) {
                  ...
                  %>
                    <tr>
									...
									<%if(rs6.getInt(11)==1)
                                                                        {%>
                                                                       <td><button type="button" class="btn btn-danger">Pending </button></a></td>
									<%}%>
                                                                        <%if(rs6.getInt(11)==2)
                                                                        {%>
                                                                       <td><button type="button" class="btn btn-green" onclick="jQuery('#<%=rs6.getInt(1)%>').modal('show', {backdrop: 'static'});">Solution</button></a></td>
			
                                                                       <div class="modal fade" id="<%=rs6.getInt(1)%>">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button> 
						<h4 class="modal-title">The Solution To The Doubt Is Given Below</h4>
					</div>
					<div class="modal-body">
						<div class="row">
							<div class="col-md-12">
                                                            <div class="form-group"> <label for="field-1" class="col-sm-3 control-label">The Solution</label> 
                                                                
                                                                <textarea class="form-control" name="sol" id="sol" readonly="true"><%=rs6.getString(10)%> </textarea>                                              
                                                            </div>
							</div>
