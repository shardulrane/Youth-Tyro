<%-- 
    Document   : viewassetrequest
    Created on : Jun 5, 2016, 8:59:39 PM
    Author     : Shardul Rane
--%>
...
//var userType= usertype .options[usertype .selectedIndex].value;
//var uname=document.search;
var urls="getrequests.jsp?stream="+str;
if (window.XMLHttpRequest)
  {
  xmlhttp=new XMLHttpRequest();
  }
...
xmlhttp.onreadystatechange=function()
  ...
  if (xmlhttp.readyState==4)
    {
        document.getElementById("showreq").innerHTML=xmlhttp.responseText;
     }
  }
xmlhttp.open("GET",urls,true);
xmlhttp.send();
...
<%
  // String stream=request.getParameter("stream");
    ...
    String cid = (String)request.getSession().getAttribute("cid");
                    ...
                    PreparedStatement ps6 = con.prepareStatement("select e_dept from register_users where cid=?  and uid=?");
                ps6.setString(1, cid);
                ps6.setString(2, uid);
                ResultSet r4=ps6.executeQuery();
                if(r4.next())
                {
                PreparedStatement ps5 = con.prepareStatement("select * from college_assets where c_id=? and stream=?  and status=1");
                ps5.setString(1, cid);
                ps5.setString(2, r4.getString("e_dept"));
