...
;(function($, window, undefined)
{
	...
	$.extend(tyroChat, {
		...
		init: function()
		{
			...
			// Conversation Close
			$conversation_window.on('click', '.conversation-close', tyroChat.close);
			...
			tyroChat.refreshUserIds();
			tyroChat.orderGroups();
			tyroChat.prefetchMessages();
			tyroChat.countUnreadMessages();
			tyroChat.puffUnreads();
			...
			$("body").on('click', '[data-toggle="chat"]', function(ev)
			{
				...
				tyroChat.toggleChat(with_animation, collapse_sidebar);
			});
		},
		...
		refreshUserIds: function()
		{
			$('#chat .chat-group a').each(function(i, el)
			{
				...
				if(typeof tyroChat.chat_history[id] == 'undefined')
				{
					var status = $this.data('status');

					if( ! status)
					{
						for(var i in tyroChat.statuses)
						{
							if($status.hasClass(tyroChat.statuses[i].class))
							{
								status = i;
								break;
							}
		...
		getStatus: function(id)
		{
			...
			var user = tyroChat.chat_history[id];
		...
		setStatus: function(id, status)
		{
			...
			if($user.length && this.statuses[status].class)
			{
				...
				tyroChat.chat_history[ $user.attr('id') ].status = status;
				this.orderGroups();
			}
		},
		...
		orderGroups: function()
		{
			...
			$groups.each(function(i, el)
			{
				...
				$contacts.each(function(i, el)
				{
					...
					for(var i in tyroChat.statuses)
					{
						var status = tyroChat.statuses[i];
			...
			$chat.find('.chat-group a').each(function(i, el)
			{
				...
				if(history_container && history_container.length)
				{
					$($(history_container)).find('> li').each(function(j, el2)
					{
						...
						tyroChat.pushMessage(id, message, from, time, fromOpponent, unread)
					});
				}
		...
		countUnreadMessages: function(id)
		{
			...
			if( ! id)
			{
				for(var id in tyroChat.chat_history)
				{
					var user = tyroChat.chat_history[id],
		...
		puffUnreads: function()
		{
			...
			{
				var entry = tyroChat.chat_history[i],
