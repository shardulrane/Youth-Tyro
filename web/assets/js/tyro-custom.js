		...
		if($.isFunction($.fn.bootstrapWizard))
		{
			$(".form-wizard").each(function(i, el)
			{
				...
				$this.bootstrapWizard({
			  	...
			  	/*$(window).on('tyro.resize', function()
			  	{
			  		$this.data('bootstrapWizard').show( _index );
			  	});*/
			});
...
function replaceCheckboxes()
{
	...
	$(".checkbox-replace:not(.tyro-cb-replacement), .radio-replace:not(.tyro-cb-replacement)").each(function(i, el)
	{
		var $this = $(el),
		...
		$this.addClass('tyro-cb-replacement');
		...
		$input.on('change', function(ev)
		{
			if(is_radio)
			{
				//$(".tyro-cb-replacement input[type=radio][name='"+name+"']").closest('.tyro-cb-replacement').removeClass(checked_class);
				$(".tyro-cb-replacement input[type=radio][name='"+name+"']:not(:checked)").closest('.tyro-cb-replacement').removeClass(checked_class);
			}
...
function show_loading_bar(options)
{
	...
	var $ = jQuery,
		$loading_bar = $(".tyro-loading-bar");

	if($loading_bar.length == 0)
	{
		$loading_bar = $('<div class="tyro-loading-bar progress-is-hidden"><span data-pct="0"></span></div>');
